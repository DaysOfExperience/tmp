...
多路复用IO用于**对大量描述符进行IO就绪事件监控，能够让用户只针对就绪了指定事件的描述符进行操作。**相比较于其他IO方式，多路复用IO避免了对没有就绪的描述符进行操作而带来的阻塞，只针对已就绪的描述符进行操作，提高了效率.
...
3. epoll的监控是一个**异步阻塞**操作，他只需要告诉操作系统哪些描述符需要监控，然后这个监控的过程就由系统来完成。
