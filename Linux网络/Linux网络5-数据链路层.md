...
而主机B到路由器F，F到G，G到H等等的局域网数据转发的问题，都是**由数据链路层在这个局域网中完成转发任务的。** 数据链路层的主要功能就是**负责局域网中相邻设备之间的数据传输 **

网络层是点对点之间的通信。主要通过IP地址描述起点到终点。而链路层则是对网络层的补充，负责在起点到终点的路径中的**每一个相邻节点的数据传输。**
...
MAC地址即用于定位局域网中相邻设备的地址，也就是物理网卡的硬件地址，标识每一块网卡，在两块网卡之间的数据传输起到作用。
