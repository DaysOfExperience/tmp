...
- change buffer 和 redo log
  ...
  >
  > 所以，如果要简单地对比这两个机制在提升更新性能上的收益的话，**redo log 主要节省的是随机写磁盘的 IO 消耗（转成顺序写），而 change buffer 主要节省的则是随机读磁盘的 IO 消耗。**(若没有change buffer, 则更新时需要随机读磁盘到内存中, 并更新, 后续读的时候是直接读内存, 就还好. 而有了change buffer, 写的时候直接写内存, 后续读是需要随机读, 并执行之前写操作, )
...
- 读未提交
  读写操作两个事务可以同时进行(且会脏读)，但写写操作无法同时进行。与此同时，该隔离级别下只会使用行级别的记录锁，并不会用间隙锁。
...
- Insert语句怎么加行级锁的?
  ...
  - 记录之间加有间隙锁(或者next-key), 此时insert, insert的事务会生成一个插入意向锁, 并发生阻塞
