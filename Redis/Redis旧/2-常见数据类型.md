...
**列表类型是用来存储多个字符串的**(字符串间的顺序有意义), 相当于数组或顺序表
...
允许重复中奖且多次抽奖(1等奖 2等奖)   `SRANDMEMBER id n`    n是x等奖的抽奖人数

不允许重复中奖 `SPOP id n` n是x等奖的抽奖人数
...
- zrange，因为本身Zset中的元素就是**有序**的，所以可以赋予元素**下标**的概念。
